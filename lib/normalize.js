...
export function updateIpHistory(ipHistory = [], currentIp, ts) {
    ...
    const existingIndex = ipHistory.findIndex(h => h.ip === currentIp);
    if (existingIndex >= 0) {
        // Only create new array if lastSeen is different
        const existing = ipHistory[existingIndex];
        if (existing.lastSeen === ts) return ipHistory;
        
        // Return a new array with the matching entry's lastSeen updated immutably
        return ipHistory.map((h, i) =>
            i === existingIndex ? { ...h, lastSeen: ts } : h
        );
    }
